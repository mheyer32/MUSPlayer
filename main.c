...
UBYTE AllNotesOff[ALLNOTESOFFLEN] = {
    MS_Ctrl | 0,  MM_AllOff, 0, MS_Ctrl | 1,  MM_AllOff, 0,
    MS_Ctrl | 2,  MM_AllOff, 0, MS_Ctrl | 3,  MM_AllOff, 0,
    MS_Ctrl | 4,  MM_AllOff, 0, MS_Ctrl | 5,  MM_AllOff, 0,
    MS_Ctrl | 6,  MM_AllOff, 0, MS_Ctrl | 7,  MM_AllOff, 0,
    MS_Ctrl | 8,  MM_AllOff, 0, MS_Ctrl | 9,  MM_AllOff, 0,
    MS_Ctrl | 10, MM_AllOff, 0, MS_Ctrl | 11, MM_AllOff, 0,
    MS_Ctrl | 12, MM_AllOff, 0, MS_Ctrl | 13, MM_AllOff, 0,
    MS_Ctrl | 14, MM_AllOff, 0, MS_Ctrl | 15, MM_AllOff, 0};
...
void main(int argc, char **argv) {
  ...
  if (pSMFHeader->Division < 0) {
    ...
    tfactor = changetempo(500000L, tfactor,
                          0L); /* One quarter note every half second */
  }
  ...
  while ((pbyte - smfdata < smfdatasize) && (trackct < MAXTRAX)) {
    if ((*pbyte == 'M') && (*(pbyte + 1) == 'T') && (*(pbyte + 2) == 'r') &&
        (*(pbyte + 3) == 'k')) {
      /* Beginning of track */
  ...
  /*----------------------------------------------*/
  pMidiNode = CreateMidi(MIDI_Name, "PlayMF Player", MIDI_MsgQueue,
                         0L,                 /* This is a send-only   */
  ...
  pMidiLink =
      AddMidiLink(pMidiNode, MLTYPE_Sender, MLINK_Comment, "PlayMF Player Link",
                  MLINK_Parse, TRUE, MLINK_Location, "out.0", TAG_END);
  if (!pMidiLink)
  ...
  fillclock[masterswitch] =
      (ULONG)(((tfactor * lowclock) + tempo_offs) >> TSHIFT);
  ...
  pPlayer = CreatePlayer(PLAYER_Name, "PlayMF Player", PLAYER_Conductor,
                         "PlayMF Conductor", PLAYER_AlarmSigTask, mt,
  ...
  /* and set the alarm.  */
  /*---------------------*/
  timerr = SetPlayerAttrs(pPlayer, PLAYER_AlarmTime, fillclock[masterswitch],
  ...
  while (donecount < trackct) {
    ...
    ylength[masterswitch] = y;
    fillclock[masterswitch] =
        (LONG)(((tfactor * lowclock) + tempo_offs) >> TSHIFT);
